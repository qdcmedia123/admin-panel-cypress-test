  ...
  useEffect(() => {
    const getUsers = async function getUsers() {
      if (
        ...
        Object.entries(usersOriginal).length === 0
      ) {
    ...
    getUsers();
  }, [apiLink, skipPage, perpage, details, users, usersOriginal]);
  let data = "";
  let searchlists = {};
  searchlists.perpage = perpage;
  if (isLoading === false) {
    searchlists.no_of_result = users.data.totalNum;
    const CopyRows = [...users.data.rows];
    var newData = CopyRows.filter(function (item) {
  ...
  const handleEvent = useCallback(
    (event, picker) => {
      ...
      var filtered = usersOriginal.data.rows.filter(function (item) {
        return (
          prevState.indexOf(item.status) !== -1 &&
  ...
  return (
    <Fragment>
      ...
      {isLoading ? (
        ...
        <Fragment>
          <WelcomeUser details={details} text="Leads USA and UAE" />
          ...
          <div className="page__section">
            <div className="page__section-item">
              <div className="filter__container">
                <div className="filter__item my__filter__item">
